        ...
        public void handleCommand(string cmd)
        {
                ...
                try
                {
                    ...
                    if(currentCommand.Contains("gfile"))
                    {
                        ...
                        DialogResult Result = MessageBox.Show($"Вы хотите принять файл {fileName} размером {Convert.ToInt32(FileSize) / 1024} от {FromName}","Файл",MessageBoxButtons.YesNo);
                        ...
                        {
                            string[] Arguments1 = currentCommand.Split('|');
                            string FromName1 = Arguments[2];
                            string FileSize1 = Arguments[3];
                            string idFile1 = Arguments[4];
                            AddMessage($"{DateTime.Now.ToString("HH:mm:ss")} | Файл {fileName} не принят от {FromName}.");
                            Send("nn");
                        }
                ...
                catch (Exception exp) { MessageBox.Show("Error with handleCommand: " + exp.Message); }
